...
class BaseSweepObject:
    ...
    def __call__(self, *objects):
        ...
        for count, obj in enumerate(objects):
            if isinstance(obj, BaseSweepObject):
                object_list.append(obj)
            elif callable(obj):
                object_list.append(FunctionWrapper(obj))
            else:
                raise ValueError("All objects in a call of sweep should either be a callable or a sweep")

        chained_sweep_object = ChainSweep(object_list)
        return NestedSweep([chained_sweep_object, self])
